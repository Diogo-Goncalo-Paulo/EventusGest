...
<div class="movement-form">
    ...
    <?= $form->field($model, 'idCredential')->widget(Select2::className(), ['options' => ['placeholder' => 'Selecione'], 'items'=>ArrayHelper::map(\common\models\Credential::find()->where(['idEvent' => Yii::$app->user->identity->getEvent()])->all(), 'id', 'ucid')]); ?>
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-4 offset-4">
                    <?= $form->field($model, 'idAccessPoint')->widget(Select2::className(), ['options' => ['placeholder' => 'Selecione'], 'items'=>ArrayHelper::map(\common\models\Accesspoint::find()->where(['id' => Yii::$app->user->identity->getAccessPoint()])->all(), 'id', 'name')]); ?>
                </div>
            </div>
            <div class="row">
                <div class="col-6">
                    <?= $form->field($model, 'idAreaFrom')->widget(Select2::className(), ['options' => ['placeholder' => 'Selecione'], 'items'=>ArrayHelper::map(\common\models\Area::find()->where(['idEvent' => Yii::$app->user->identity->getEvent()])->all(), 'id', 'name')]); ?>
                </div>
                <div class="col-6">
                    <?= $form->field($model, 'idAreaTo')->widget(Select2::className(), ['options' => ['placeholder' => 'Selecione'], 'items'=>ArrayHelper::map(\common\models\Area::find()->where(['idEvent' => Yii::$app->user->identity->getEvent()])->all(), 'id', 'name')]); ?>
                </div>
            </div>

            <div class="form-group mb-0">
                <?= Html::submitButton('Registar movimento', ['class' => 'btn btn-success btn-block']) ?>
            </div>
        </div>
    </div>
            ...
            <div class="row">
                ...
                <div class="col-4">
                    <div class="mt-5">
                        ...
                        <button id="credBlock" data-credId="0" data-credAction="block" class="btn btn-sm btn-action" data-toggle="tooltip" title="Bloquear/Desbloquear">
